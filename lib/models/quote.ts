...
export async function createQuote(input: CreateQuoteInput): Promise<Quote> {
    ...
    const result = await db.transaction(async (tx) => {
        const [insertedQuote] = await tx.insert(quotes).values({
            ...
            delivery_weeks: input.delivery_weeks ?? null,
    ...
    await db.transaction(async (tx) => {
        if (input.items) {
            ...
            await tx.update(quotes).set({
                ...
                delivery_weeks: input.delivery_weeks !== undefined ? (input.delivery_weeks ?? null) : existing.delivery_weeks,
                issue_date: input.issue_date !== undefined ? (input.issue_date ?? null) : existing.issue_date,
                total,
