import { Check, X, Clock, FileEdit, Send, ThumbsUp } from 'lucide-react';
...
export default function QuoteProgressBar({ status, createdAt, updatedAt }: QuoteProgressBarProps) {
    ...
    if (status === 'rejected') {
        steps[2] = { id: 'rejected', label: 'Rejected', icon: X };
    } else if (status === 'expired') {
        steps[2] = { id: 'expired', label: 'Expired', icon: Clock };
    }
    ...
    const getStepTime = (index: number) => {
        ...
        // So we'll just show a dash or nothing
        return '—';
    };
